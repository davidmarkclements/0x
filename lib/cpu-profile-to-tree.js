...
function id2IndexMap (nodes) {
  let map = new Array(nodes.length + 1)
...
function convert_cpu_prof (nodes, id2Index, id) {
  ...
  for (let i = 0; i < childrenLength; i++) {
    converted.children[i] = convert_cpu_prof(nodes, id2Index, node.children[i])
    converted.value += converted.children[i].value
  }
  return converted
}
...
function cpuProfileToTree (profile) {
  const converted = profile.head ?
    ...
    convert_cpu_prof(profile.nodes, id2IndexMap(profile.nodes), 1)
  return {
    merged: converted,
    unmerged: converted
  }
