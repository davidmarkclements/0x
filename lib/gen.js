...
var ui = require('d3-fg')
module.exports = function (stacks, opts, next, done) {
  opts = opts || {}

  if (!opts.htmlPath) opts.stdout = true
  if (opts.htmlPath === '-') opts.stdout = true
...

  {
    ...
    function genBox (d) {
      ...
      var style = 'background: ' + ui.colorHash(d, 1, 100) + ';'
  ...
  if (!browser) {
    ...
    if (opts.stdout) {
      ...
      fs.writeFileSync(opts.htmlPath,
    ...
    if (opts.svg) {
      ...
      debug('writing svg')
      fs.writeFileSync(dir + '/flamegraph.svg', '<?xml version="1.0" encoding="UTF-8" standalone="no"?>\n' + chart.innerHTML)
      next() && next()
      done() && done()
    } else {
      console.log('\n')
