...
function tickStacksToTree (stacks, mapFrames) {
  ...
  stacks.forEach((stack) => {
    ...
    stack = stack.map(({name, kind}) => {
      if (name[0] === ' ') name = '(anonymous)' + name
      const fn = name
      if (kind === 'Opt') name = '*' + name
      if (kind === 'Unopt') name = '~' + name 

      const frame = new String(name)
      frame.valueOf = frame.valueOf.bind(frame)
      frame.toString = frame.toString.bind(frame)
      return {__proto__: frame, fn, name, value: 0, top: 0, children: {__proto__: {toJSON: values}}}
    })
