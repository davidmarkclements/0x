...
async function render (args, {json, pid, folder, clear}) {
  ...
  return new Promise((resolve, reject) => {
    gen(json, opts, clear || noop, function (err) {
      if (err) return void reject(err)
      const file = htmlPath === '-' ? null : `file://${htmlPath}`
...
async function pathTo (bin) {
  if (fs.existsSync(bin)) { return bin }
  var path
  try { path = which.sync(bin) } catch (err) {
    throw err
  }
  return path
}
