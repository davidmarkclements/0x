...
const phases = [
  (frames) => frames,
  (frames) => {
    var moduleRunMain = frames.find(({fn}) => /Module.runMain module.js/.test(fn))
    if (moduleRunMain) {
      const startupIndex = frames.findIndex(({fn}) => /startup bootstrap_node/.test(fn))
      if (startupIndex !== -1) frames.splice(startupIndex, 1)
      return frames
    } 

    var startup = frames.find(({fn}) => /startup bootstrap_node/.test(fn))
    if (startup) return false
    return frames
  },
  (frames) => {
    if (frames.find(({fn}) => /startup bootstrap_node/.test(fn))) return false
    return frames
  }
]
...
async function render (args, {tree, pid, folder}) {
  ...
  const script = `${await createBundle()}\nvisualizer(${JSON.stringify(tree)}, ${JSON.stringify({title})})`
