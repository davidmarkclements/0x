...
async function ticksToTree (ticks, options = {}) {
  ...
  if (sourceMaps) {
    const mapper = async (jsLibrary) => { return loadSourceMap(jsLibrary, sourceMaps) }
...
function renameFrame (options, frame) {
  if (frame.type === 'JS') {
  ...
  if (frame.children && frame.children.length) {
    frame.children.forEach((child) => renameFrame(options, child))
  }
  delete frame.type
}
