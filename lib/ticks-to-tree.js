...
const { accessSync, readFile } = require('fs')
...
async function ticksToTree (ticks, options = {}) {
  ...
  if (sourceMaps || relativePath) {
    for (var i = 0; i < Object.keys(trees).length; i++) {
      for (let j = 0; j < Object.keys(trees)[i].children.length; j++) {
        let frame = Object.keys(trees)[i].children[j]
        renameFrame(options, frame, loadedSourceMaps)
      }
    }
  }
  for (let x = Object.keys(loadedSourceMaps).length - 1; x >= 0; x--) {
    let file = Object.keys(loadedSourceMaps)[i]
  ...
  if (typeof loadedSourceMaps[fileName] === 'undefined') {
    ...
    for (let i = 0; i < sourceMapFiles.length; i++) {
      if (sourceMapFiles[i]) {
        ...
        if (accessSync(sourceMapFiles[i])) {
          try {
...
function addToMergedTree (trees, stack) {
  ...
  for (var i = 0; i < stack.length; i++) {
    ...
    if (stack[i].S > 2) return // skip inlined
    if (i > 0) lastFrame.children = lastFrame.children || []
    const children = (i === 0) ? trees.merged.children : lastFrame.children
    const child = children.find(({ name }) => name === stack[i].name)

    if (child === undefined) children.push(stack[i])
    else stack[i] = child

    if (i === stack.length - 1) stack[i].top++
    if (i === 0) trees.merged.value += 1
    stack[i].value++
...
function addToUnmergedTree (trees, stack) {
  ...
  for (var i = 0; i < stack.length; i++) {
    ...
    if (i > 0) lastFrame.children = lastFrame.children || []
    const children = (i === 0) ? trees.unmerged.children : lastFrame.children
    const child = children.find(({ fn, S }) => {
      return fn === stack[i].name && S === stack[i].S
    })

    if (child === undefined) {
      stack[i].fn = stack[i].name
      if (stack[i].S === 1) stack[i].name = '~' + stack[i].name
      else if (stack[i].S === 2) stack[i].name = '*' + stack[i].name
      else if (stack[i].S === 3) stack[i].name = '~' + stack[i].name + ' [INLINABLE]'
      else if (stack[i].S === 4) stack[i].name = '*' + stack[i].name + ' [INLINABLE]'
      children.push(stack[i])
    } else stack[i] = child

    if (i === stack.length - 1) stack[i].top++
    if (i === 0) trees.unmerged.value += 1
    stack[i].value++
